  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      providers: [
        ...
        // Data providers below all listen to AuthController.uid via
        // updateAuth(). Each one automatically starts/stops its Firestore
        // stream when the user signs in or out — no screen has to pass
        // uid around manually.
        ChangeNotifierProxyProvider<AuthController, ApplicationProvider>(
          create: (_) => ApplicationProvider(),
          update: (_, auth, provider) =>
          provider!..updateAuth(auth.uid),
        ),
        ...
        // StatisticsController is derived purely from ApplicationProvider's
        // live list — no separate repository call needed, so it must be
        // declared after ApplicationProvider above.
