...
class _AddApplicationScreenState extends State<AddApplicationScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final model = ApplicationModel(
        // TODO(step-2 Core Data): id should come from
        // ApplicationsRepository.create(uid, model) (Firestore doc id).
        // Using a client-side timestamp id as a temporary placeholder so
        // the model can be constructed before the repository exists.
      ...
      await context.read<ApplicationProvider>().create(model);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(context.tr('add_application_title'))),
      body: SafeArea(
        child: Center(
                ...
                children: [
                  ...
                  DropdownButtonFormField<ApplicationStatus>(
                    ...
                    items: ApplicationStatus.values
                        ...
                        value: s, child: Text(context.tr('status_${s.name}'))))
                        .toList(),
